...
data_1001$time <- 1:nrow(data_1001)
...
sqrt(mean((training$count-p)^2,na.rm=T))/sqrt(mean((training$count)^2))

# AR lag 2 model
mod.l2 <- gam(training$count ~ s(count2,k=4), 
              family=quasipoisson, na.action=na.exclude)
summary(mod.l2)
...
plot(training$time, training$count, type="l",ylab="Number Cases",axes=T,xlab="Year")
...
# dlag <- data.frame()
# 
# cb1.temp <- crossbasis(training$count1, lag=31, argvar=list(df=4), arglag=list(fun="poly",degree=4))
# 
# summary(cb1.temp)
# 
# mod.dlnm <- glm(count ~  cb1.temp, family=quasipoisson(), training)
# 
...
mod.fcum <- gam(count ~ s(count2,k=4) + s(count4,k=4), 
...
mod.offset <- gam(count ~ s(templ3,k=4) + s(rainl2,k=4) + s(rainl3,k=4) + s(count2,k=4) + s(count3,k=4) + offset(log(Community)) + offset(log(ppl))  , family=quasipoisson, na.action=na.exclude, data = training)
...
```{r fillingMissingValues, echo=FALSE, results="hide", include=FALSE}
library(dplyr)
library(tidyr)



a<- data_1001 %>%  group_by(geocode_district,date_sick_year, district_name,
                            ...
                            X25.29, X30.34, X35.39, 
                            X40.44, X45.49, X50.54, X55.59, X60.64, X65.69, X70.74, X75.79, X80.84, X85.89, X90.94, X95.99, X..100, neighbors, time ) %>% complete(date_sick_month = 1:12)
